...
const random = (min, max) => {
  ...
  window.addEventListener('message', function(event) {
    ...
    if (item.showUI) {
          $("input").attr({
            "max" : item.size,        // substitute your own
            "min" : item.size          // values (or variables) here
        });
        numbers = item.size;
        $('html').show();
    }
  ...
  document.onkeyup = function (data) {
    if (data.which == 27) {
      $.post('http://erp_memorygame/MemoryResult', JSON.stringify({
        success: false
      }));
  ...
  document.querySelector('.successending').classList.remove('hidden');
  setTimeout(function(){
    $.post('http://erp_memorygame/MemoryResult', JSON.stringify({
      success: true
    }));
  ...
  document.querySelector('.minigame').classList.add('hidden')
  document.querySelector('.failending').classList.remove('hidden');
  setTimeout(function(){
    $.post('http://erp_memorygame/MemoryResult', JSON.stringify({
      success: false
    }));
